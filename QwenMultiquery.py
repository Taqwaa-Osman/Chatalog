...
class MultiQueryRAGQwen:
    ...
    def _verify_ollama(self):
        try:
            ...
            if not any('qwen2.5:3b' in name.lower() for name in model_names):
                raise RuntimeError("Qwen2.5 not found. Run: ollama pull qwen2.5:3b")
        except Exception as e:
    ...
    def generate_query_variations(self, query: str, num_variations: int = 3) -> List[str]:
        """Generate multiple query variations using Qwen2.5:3b."""
...
def main():
    ...
    system = MultiQueryRAGQwen(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, DATABASE)
