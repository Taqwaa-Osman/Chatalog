...
class VectorRAGMinistral3:
    ...
    def _verify_ollama(self):
        try:
            ...
            if not any('ministral-3:3b' in name.lower() for name in model_names):
                raise RuntimeError("ministral-3:3b not found. Run: ollama pull Ministral3")
        except Exception as e:
    ...
    def _precompute_embeddings(self, max_books: int = 5000):
        """Pre-compute embeddings for books. Uses more books for better coverage."""
    ...
    def retrieve_books_by_vector(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        """Core method: Vector similarity search with AND/OR filtering."""
...
def main():
    ...
    system = VectorRAGMinistral3(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, DATABASE)
    
    try:
        print("\nChatalog - Seattle Public Library Assistant")
        print("Powered by Vector RAG\n")
