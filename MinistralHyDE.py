...
class HydeRAGMinistral3:
    ...
    def _verify_ollama(self):
        try:
            ...
            if not any('ministral-3:3b' in name.lower() for name in model_names):
                raise RuntimeError("ministral-3 not found. Run: ollama pull ministral-3:3b")
        except Exception as e:
    ...
    def generate_hypothetical_document(self, query: str) -> str:
        ...
        prompt = f"""Describe in 2-3 sentences what kind of books would match this request: "{query}"
Include themes, genres, and typical characteristics."""

        try:
            response = ollama.generate(model=self.model_name, prompt=prompt, options={"temperature": 0.3})
...
def main():
    ...
    system = HydeRAGMinistral3(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, DATABASE)
    
    try:
        print("\nChatalog - Seattle Public Library Assistant")
        print("Powered by HyDE RAG\n")
