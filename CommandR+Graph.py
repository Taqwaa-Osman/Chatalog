...
class GraphRAGCommandR:
    ...
    def __init__(self, neo4j_uri: str, neo4j_user: str, neo4j_password: str, database: str = "neo4j"):
        ...
        self.model_name = "command-r-plus-08-2024"
    ...
    def _verify_ollama(self):
        try:
            ...
            if not any('command-r-plus-08-2024' in name.lower() for name in model_names):
                raise RuntimeError("Command R Plus not found. Run: cohere pull command-r-plus-08-2024")
        except Exception as e:
    ...
    def generate_response(self, query: str, context: str, books: List[Dict[str, Any]] = None, stream: bool = False) -> str:
        """Generate response using Command R + with Chatalog persona."""
        if not books:
...
def main():
    ...
    system = GraphRAGCommandR(NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD, DATABASE)
