...
class ExperimentRunner: 
    ...
    def load_rag_system(self, rag_name: str):
        ...
        if rag_name == "graphrag":
            ...
            system = GraphRAGQwen(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_keyword
        
        elif rag_name == "hyde":
            from QwenHyDE import HydeRAGQwen
            system = HydeRAGQwen(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_hyde
        
        elif rag_name == "subgraph":
            from QwenSubgraph import SubGraphRAGQwen
            system = SubGraphRAGQwen(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_subgraph
        
        elif rag_name == "vector":
            from QwenVector import VectorRAGQwen
            system = VectorRAGQwen(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_vector
        
        elif rag_name == "multiquery":
            from QwenMultiquery import MultiQueryRAGQwen
            system = MultiQueryRAGQwen(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
