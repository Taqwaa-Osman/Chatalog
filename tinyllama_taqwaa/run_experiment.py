...
class ExperimentRunner:
    ...
    def load_rag_system(self, rag_name: str):
        ...
        if rag_name == "graphrag":
            ...
            system = GraphRAGTinyLlama(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_keyword
        
        elif rag_name == "hyde":
            from hyderag_tinyllama import HydeRAGTinyLlama
            system = HydeRAGTinyLlama(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_hyde
        
        elif rag_name == "subgraph":
            from subgraphrag_tinyllama import SubGraphRAGTinyLlama
            system = SubGraphRAGTinyLlama(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_subgraph
        
        elif rag_name == "vector":
            from vectorrag_tinyllama import VectorRAGTinyLlama
            system = VectorRAGTinyLlama(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
            return system, system.retrieve_books_by_vector
        
        elif rag_name == "multiquery":
            from multiqueryrag_tinyllama import MultiQueryRAGTinyLlama
            system = MultiQueryRAGTinyLlama(self.neo4j_uri, self.neo4j_user, self.neo4j_password, self.database)
...
Reply with only a number:"""
        try:
            response = ollama.generate(model="tinyllama", prompt=prompt, options={"temperature": 0.1})
